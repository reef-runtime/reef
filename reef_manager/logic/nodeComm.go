...
func (m *NodeManagerT) jobStartedJobCallbackInternal(nodeID NodeID, message []byte) (jobID JobID, err error) {
	...
	if currWorkerJobID == nil {
		return "", fmt.Errorf(
			"worker index returned from node (%d) is not busy with job, is <nil> -> node sent bogus value",
			workerIndex,
		)
	}
	...
	if *currWorkerJobID != jobID {
		return "", fmt.Errorf("job ID returned from node (%s) != expected `%s` -> node sent bogus value",
			jobID,
			*currWorkerJobID,
		)
	}
	...
	log.Debugf(
		"[node] Received callback from node, started job `%s` on node `%s`",
		jobID, FormatBinarySliceAsHex(nodeID[:]),
	)
