            ...
            <CardDescription className="text-muted-foreground pt-2">
                <div className='flex justify-between w-full'>
                    <div className='w-1/2 mr-4 flex flex-col'>
                            <span className='text-nowrap text-xs'>{`Load: ${node.workerState.filter(w => w).length} / ${node.info.numWorkers} worker${node.info.numWorkers === 1 ? '' : 's'}`}</span>
                            <span className='text-ellipsis overflow-hidden w-full text-nowrap text-xs'>ID: {`${node.id}`}</span>
                    </div>
                    <Separator orientation='vertical'></Separator>
                    <div className='w-1/2 ml-4 flex flex-col'>
                        <span className='text-nowrap text-xs'>IP: {`${node.info.endpointIP}`}</span>
