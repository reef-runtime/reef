...
const WorkerListItem: React.FC<WorkerListItemProps> = ({ job, workerIndex }) => {
    return (
    <div className="w-full overflow-hidden">
      <ul
        key={workerIndex}
        className={classNames('space-y-2 p-2 rounded-xl transition-shadow duration-300 hover:shadow-lg cursor-pointer h-12', {
        ...
        <li className="text-sm text-muted-foreground font-bold flex items-center space-x-1">
          ...
          <span className='text-sm font-medium leading-none'>
          {
            function() {
                ...
                return <span className='text-sm font-medium leading-none'>{percentage} %</span>
            }()
          }
          </span>
        </li>
